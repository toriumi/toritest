...
IplImage* BayerImg, *RGBImg, *RGBSmallImg;
...
void fame_preprocess(struct ssp_handle *handle, struct ssp_frame *frame){
  ...
  for(i=0;i<NUM_COPY_THREAD;i++)
    ...
    // Resize for detect
    cvResize(RGBImg, RGBSmallImg);
    // Detect faces
    std::vector<Rect> faces;
    face_cascade.detectMultiScale( RGBSmallImg, faces, 1.1, 2, 0|CV_HAAR_SCALE_IMAGE, Size(30, 30) );

    for( i = 0; i < faces.size(); i++ )
    {
      //Point center( faces[i].x + faces[i].width*0.5, faces[i].y + faces[i].height*0.5 );
      Point lefttop ( faces[i].x*((float)1/resize_ratio), faces[i].y*((float)1/resize_ratio));
      Point rightbottom ( (faces[i].x + faces[i].width)*((float)1/resize_ratio),
			  (faces[i].y + faces[i].height)*((float)1/resize_ratio));
        cvRectangle( RGBImg, lefttop, rightbottom, Scalar( 255, 0,0 ), 1);
...
int main(int argc, char **argv){
  ...
  if(argc < 3){
    printf("SSP library face detection sample program\n");
  ...
  resize_ratio = (float)DETECT_IMAGE_RATIO/100;
  ...
  RGBSmallImg = cvCreateImage(cvSize(profile->ImageProperty.Width*resize_ratio,profile->ImageProperty.Height*resize_ratio),IPL_DEPTH_8U,3);
